/**
 * InvoiceModal — كومبوننت مشترك لمودال الفاتورة
 *
 * Props:
 *  - show      : boolean
 *  - onClose   : دالة الإغلاق
 *  - title     : عنوان المودال  (مثلاً "فاتورة شراء قطع غيار")
 *  - invoice   : {
 *      id      : رقم الفاتورة,
 *      meta    : [{ label, value }, ...] — بيانات الهيدر (4 عناصر),
 *      items   : [{ name, supplier?, qty, unitPrice }, ...]
 *    }
 */
...
const InvoiceModal = ({ show, onClose, title, invoice }) => {
  if (!show) return null;
      ...
      >
        ...
        <div
          style={{
            ...
            padding: "20px 24px",
            display: "grid",
            gridTemplateColumns: `repeat(${invoice?.meta?.length ?? 4}, 1fr)`,
            gap: "16px",
            marginBottom: "24px",
            textAlign: "right",
          }}
        >
          {invoice?.meta?.map((item, i) => (
            <div key={i}>
        ...
        {/* جدول القطع */}
        <div style={{ backgroundColor: "#F8F9FB", borderRadius: "12px", overflow: "hidden", marginBottom: "16px" }}>
